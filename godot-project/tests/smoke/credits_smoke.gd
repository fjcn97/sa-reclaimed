...
func _run() -> void:
	...
	_check(bridge.get_credits_end_source_tilemap() == "credits_sa2_logo_en", "final ending uses original localized logo card")
	bridge._ending_variant = bridge.ENDING_VARIANT_EXTRA
	bridge._open_credits_end()
	_check(bridge.get_credits_end_source_tilemap() == "storyframe_sonic_leaves_0", "extra ending starts on original storyframe card")
	bridge.advance_ui_timers(0.01)
	_check(bridge.get_credits_end_source_tilemap() == "storyframe_sonic_leaves_1", "extra ending advances to storyframe one")
	bridge.advance_ui_timers(0.049)
	_check(bridge.get_credits_end_source_tilemap() == "storyframe_sonic_leaves_1", "storyframe one holds for three source frames")
	bridge.advance_ui_timers(0.01)
	bridge.advance_ui_timers(0.01)
	_check(bridge.get_credits_end_source_tilemap() == "storyframe_sonic_leaves_2", "extra ending follows source storyframe timing")
