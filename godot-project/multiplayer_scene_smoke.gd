...
func _run() -> void:
	...
	await process_frame
	_check(main.get_node_or_null("MultiplayerModeScreen") != null, "Main scene contains the multiplayer mode screen")
	bridge.open_multiplayer_comm_screen(0)
	await process_frame
	_check(main.get_node_or_null("MultiplayerCommScreen") != null, "Main scene contains the communication screen")
	bridge.open_multiplayer_outcome_screen(0, bridge.TITLE_PHASE_MULTI_CONNECT)
	await process_frame
	_check(main.get_node_or_null("MultiplayerOutcomeScreen") != null, "Main scene contains the connection outcome screen")
	bridge.open_multiplayer_lobby_screen(0)
	await process_frame
	_check(main.get_node_or_null("MultiplayerLobbyScreen") != null, "Main scene contains the rematch lobby screen")
