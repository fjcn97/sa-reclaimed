...
func _run() -> void:
	...
	_check(bridge._name_entry_matrix_page_index == 99, "non-Japanese starts on source matrix page")
	...
	_check(bridge._name_entry_cursor_row == 6 and bridge._name_entry_matrix_page_index == 165, "up wraps to final matrix page")
	bridge.move_save_selection(1)
	...
	bridge._name_entry_cursor_row = 4
	bridge.adjust_save_selection(-1)
	_check(bridge.is_name_entry_control_cursor() and bridge._name_entry_cursor_col == 11, "left reaches controls from control row")
	bridge.adjust_save_selection(1)
