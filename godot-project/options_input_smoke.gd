...
func _run() -> void:
	...
	_check(bridge.get_difficulty_rows()[1]["status"] == "WENIGER GEGNERDRUCK", "difficulty row status follows localization")
	...
	_check(bridge._difficulty_index != difficulty_before, "difficulty preview changes before confirm")
	bridge.cancel_save_selection()
	_check(bridge._difficulty_index == difficulty_before, "B cancels difficulty preview")
	bridge.open_options_screen()
	bridge._options_menu_index = 1
	bridge.accept_save_selection()
	bridge.adjust_save_selection(1)
	bridge.accept_save_selection()
	_check(bridge._difficulty_index != difficulty_before, "A commits difficulty change")
	bridge.open_options_screen()
	bridge._options_menu_index = 2
	var time_limit_before: bool = bridge._time_limit_enabled
	bridge.accept_save_selection()
	bridge.adjust_save_selection(1)
	_check(bridge._time_limit_enabled != time_limit_before, "time limit preview changes before confirm")
	bridge.cancel_save_selection()
	_check(bridge._time_limit_enabled == time_limit_before, "B cancels time limit preview")
