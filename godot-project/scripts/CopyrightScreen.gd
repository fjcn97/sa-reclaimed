...
func _ready() -> void:
	...
	_ensure_chrome()
	_set_screen_visible(_bridge != null and _bridge.is_copyright_screen())

func _process(delta: float) -> void:
	if _bridge == null:
		_bridge = resolve_state_bridge()
	var active: bool = _bridge != null and _bridge.is_copyright_screen()
	...
	if title_label:
		title_label.text = _bridge.get_copyright_title_text()
		title_label.modulate = Color(0.98, 0.98, 1.0, 1.0)
	if prompt_label:
		prompt_label.text = _bridge.get_copyright_prompt_text()
		prompt_label.modulate = Color(1.0, 0.88, 0.40, 0.86 + pulse * 0.14)
	if detail_label:
		detail_label.text = _bridge.get_copyright_detail_text()
...
func _ensure_chrome() -> void:
	_backdrop = ensure_rect("BackdropShade", Rect2(0.0, 0.0, 1280.0, 720.0), Color(0.01, 0.02, 0.05, 0.98))
	_panel = ensure_rect("CopyrightPanel", Rect2(188.0, 150.0, 904.0, 404.0), Color(0.05, 0.10, 0.20, 0.97))
	_rule = ensure_rect("CopyrightRule", Rect2(286.0, 286.0, 708.0, 5.0), Color(0.28, 0.72, 1.0, 0.72))
	_seal = ensure_rect("CopyrightSeal", Rect2(550.0, 352.0, 180.0, 94.0), Color(0.20, 0.52, 0.86, 0.28))
	_backdrop.z_index = -4
	_panel.z_index = -3
	_rule.z_index = -2
	_seal.z_index = -1
