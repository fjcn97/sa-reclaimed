...
func _process(_delta: float) -> void:
	if _bridge == null:
		_bridge = resolve_state_bridge()
	var screen_visible: bool = _bridge != null and _bridge.is_time_attack_lobby_screen()
	_set_screen_visible(screen_visible)
	if not screen_visible:
		return
	var pulse := 0.5 + (sin(Time.get_ticks_msec() / 220.0) * 0.5)
	if title_label:
		title_label.text = _bridge.get_time_attack_lobby_title()
	...
	if prompt_label:
		prompt_label.text = _bridge.get_time_attack_lobby_prompt()
	...
	if detail_label:
		detail_label.text = _bridge.get_time_attack_lobby_detail()
...
func _update_summary() -> void:
	var colors: Dictionary = _bridge.get_time_attack_lobby_chrome_colors()
	var accent: Color = _bridge.get_time_attack_lobby_character_accent_color()
	_focus_slide = lerpf(_focus_slide, float(_bridge.get_time_attack_lobby_cursor()) * 6.0, clampf(get_process_delta_time() * 8.0, 0.0, 1.0))
	if _emblem_label:
		_emblem_label.text = _bridge.get_time_attack_lobby_emblem_text()
		_emblem_label.modulate = Color(0.98, 0.98, 1.0, 1.0)
	if _character_label:
		_character_label.text = _bridge.get_time_attack_lobby_character_text()
		_character_label.modulate = Color(0.96, 0.98, 1.0, 1.0)
	if _course_label:
		_course_label.text = _bridge.get_time_attack_lobby_course_text()
		_course_label.modulate = Color(0.72, 0.84, 0.98, 0.94)
	if _mode_label:
		_mode_label.text = _bridge.get_time_attack_lobby_mode_text()
		_mode_label.modulate = Color(1.0, 0.88, 0.40, 1.0)
	if _record_label:
		_record_label.text = _bridge.get_time_attack_lobby_record_label_text()
...
func _update_rows() -> void:
	var rows: Array = _bridge.get_time_attack_lobby_rows()
	var colors: Dictionary = _bridge.get_time_attack_lobby_chrome_colors()
