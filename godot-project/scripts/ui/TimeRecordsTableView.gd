...
func setup(screen: ScreenBase) -> void:

	if summary_label == null:
		summary_label = screen.ensure_label("SummaryLabel", Vector2(228.0, 282.0), Vector2(224.0, 82.0), 15)
		summary_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
	if character_label == null:
		character_label = screen.ensure_label("CharacterLabel", Vector2(228.0, 434.0), Vector2(224.0, 28.0), 20)
		character_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
	if heading_label == null:
		heading_label = screen.ensure_label("HeadingLabel", Vector2(556.0, 278.0), Vector2(462.0, 28.0), 22)
	if subtitle_label == null:
		subtitle_label = screen.ensure_label("SubtitleLabel", Vector2(556.0, 308.0), Vector2(462.0, 22.0), 14)
	if character_caption == null:
		character_caption = screen.ensure_label("CharacterCaption", Vector2(244.0, 320.0), Vector2(204.0, 42.0), 24)
		character_caption.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	if row_labels.size() == 0:
		for i in range(5):
			var top := 346.0 + float(i) * 30.0
			var card := screen.ensure_rect("RowCard%d" % i, Rect2(548.0, top, 494.0, 24.0), Color(0.10, 0.16, 0.29, 0.96))
			var row := screen.ensure_label("RowLabel%d" % i, Vector2(572.0, top - 1.0), Vector2(176.0, 26.0), 16)
			var time := screen.ensure_label("TimeLabel%d" % i, Vector2(766.0, top - 1.0), Vector2(248.0, 26.0), 16)
	...
	if badge_label == null:
		badge_label = screen.ensure_label("BadgeLabel", Vector2(888.0, 160.0), Vector2(106.0, 40.0), 18)
		badge_label.text = "RECORD"
		badge_label.modulate = Color(1.0, 0.95, 0.74, 0.95)

## The screen supplies its immutable display state. This class owns table
## layout only and never reads the CoreBridge autoload.
func update(view_state: Dictionary) -> void:
	var time_attack_context: bool = bool(view_state.get("time_attack_context", false))
	var summary_text: String = str(view_state.get("summary_text", ""))
	var character_text: String = str(view_state.get("character_text", ""))
	var rows: Array = view_state.get("rows", [])
	var best_labels: Array = view_state.get("best_labels", [])
	if summary_label:
		summary_label.text = summary_text.replace("\n", "\n\n") if time_attack_context else summary_text.replace("\n", "   ")
		summary_label.modulate = Color(0.90, 0.96, 1.0, 0.98)
	if character_label:
		character_label.text = character_text
		character_label.modulate = Color(0.78, 0.88, 1.0, 1.0)
	if heading_label:
		heading_label.text = str(view_state.get("heading_text", ""))
		heading_label.modulate = Color(0.98, 0.98, 1.0, 1.0)
	if subtitle_label:
		subtitle_label.text = str(view_state.get("subtitle_text", ""))
		subtitle_label.modulate = Color(0.72, 0.84, 1.0, 0.96)
	if character_caption:
		character_caption.text = character_text
		character_caption.modulate = Color(1.0, 0.97, 0.84, 0.98) if time_attack_context else Color(0.80, 0.90, 1.0, 0.0)
	if badge_label:
		badge_label.text = str(view_state.get("badge_text", "RECORD"))
		badge_label.modulate = Color(1.0, 0.95, 0.74, 0.95)
	...
	for i in range(row_labels.size()):
		...
		row_labels[i].text = str(best_labels[i]) if time_attack_context and not mode_choice_view and i < best_labels.size() else str(row.get("name", ""))
