...
static func selected_name(bridge: Object) -> String:
	return bridge._character_names[bridge._selected_character_index]

static func description(bridge: Object, index: int) -> String:
	match clampi(index, 0, bridge._character_descriptions.size() - 1):
		0:
			return bridge._language_text("BALANCED SPEED TYPE", "AUSGEGLICHENER TEMPO-TYP", "TYPE VITESSE EQUILIBRE", "TIPO VELOCIDAD EQUILIBRADO", "TIPO VELOCITA BILANCIATO")
		1:
			return bridge._language_text("FLIGHT AND CHEESE SUPPORT", "FLUG UND CHEESE-HILFE", "VOL ET SOUTIEN DE CHEESE", "VUELO Y APOYO DE CHEESE", "VOLO E SUPPORTO DI CHEESE")
		2:
			return bridge._language_text("FLIGHT AND TECHNICAL ROUTES", "FLUG UND TECHNISCHE WEGE", "VOL ET PARCOURS TECHNIQUES", "VUELO Y RUTAS TECNICAS", "VOLO E PERCORSI TECNICI")
		3:
			return bridge._language_text("POWER AND CLIMB ROUTES", "KRAFT UND KLETTERWEGE", "FORCE ET PARCOURS VERTICAUX", "FUERZA Y RUTAS VERTICALES", "FORZA E PERCORSI VERTICALI")
	return bridge._language_text("HAMMER TECHNIQUE", "HAMMER-TECHNIK", "TECHNIQUE DU MARTEAU", "TECNICA DEL MARTILLO", "TECNICA DEL MARTELLO")

static func status_text(bridge: Object, index: int) -> String:
	if not bridge.is_character_select_character_available(index):
		return bridge._language_text("LOCKED", "GESPERRT", "VERROUILLE", "BLOQUEADO", "BLOCCATO")
	if index == bridge.CHARACTER_NAMES_AMY_INDEX():
		return bridge._language_text("SPECIAL", "SPEZIAL", "SPECIAL", "ESPECIAL", "SPECIALE")
	return bridge._language_text("READY", "BEREIT", "PRET", "LISTO", "PRONTO")
...
static func rows(bridge: Object) -> Array:
	...
	for index_variant in bridge.get_character_select_available_indices():
		...
		result.append({"name": str(bridge._character_names[index]), "description": description(bridge, index), "status": status_text(bridge, index), "available": bridge.is_character_select_character_available(index), "selected": index == bridge._selected_character_index})
	return result

static func summary_text(bridge: Object) -> String:
	return "%s\n%s\n\n%s\n%s\n\n%s\n%s" % [bridge._language_text("RUNNER", "LAEUFER", "COUREUR", "CORREDOR", "CORRIDORE"), selected_name(bridge), bridge._language_text("STYLE", "STIL", "STYLE", "ESTILO", "STILE"), description(bridge, bridge._selected_character_index), bridge._language_text("STATE", "STATUS", "ETAT", "ESTADO", "STATO"), status_text(bridge, bridge._selected_character_index)]
...
static func context_label(bridge: Object) -> String:
	match bridge._character_select_context:
		bridge.CHARACTER_SELECT_CONTEXT_TIME_ATTACK_ZONE:
			return bridge._language_text("ZONE ATTACK", "ZONEN-ANGRIFF", "ATTAQUE ZONE", "ATAQUE DE ZONA", "ATTACCO ZONA")
		bridge.CHARACTER_SELECT_CONTEXT_TIME_ATTACK_BOSS:
			return bridge._language_text("BOSS ATTACK", "BOSS-ANGRIFF", "ATTAQUE BOSS", "ATAQUE DE JEFE", "ATTACCO BOSS")
		bridge.CHARACTER_SELECT_CONTEXT_MULTIPLAYER:
...
static func title_text(bridge: Object) -> String:
	match bridge._character_select_context:
		...
		bridge.CHARACTER_SELECT_CONTEXT_MULTIPLAYER:
			return bridge._language_text("MULTIPLAYER", "MEHRSPIELER", "MULTIJOUEUR", "MULTIJUGADOR", "MULTIGIOCATORE")
	return bridge._language_text("CHARACTER SELECT", "CHARAKTERWAHL", "CHOIX DU PERSONNAGE", "SELECCION DE PERSONAJE", "SCELTA PERSONAGGIO")
...
static func prompt_text(bridge: Object) -> String:
	...
	match bridge._character_select_context:
		bridge.CHARACTER_SELECT_CONTEXT_TIME_ATTACK_ZONE:
			return bridge._language_text("SELECT A RUNNER", "WAHLE EINEN LAUFER", "CHOISISSEZ UN COUREUR", "ELIGE UN CORREDOR", "SCEGLI UN CORRIDORE")
		bridge.CHARACTER_SELECT_CONTEXT_TIME_ATTACK_BOSS:
...
static func detail_text(bridge: Object) -> String:
	...
	match bridge._character_select_context:
		bridge.CHARACTER_SELECT_CONTEXT_TIME_ATTACK_ZONE:
			return description_text + "\n" + bridge._language_text("ZONE RECORD ATTACK", "ZONEN-REKORDANGRIFF", "RECORD ZONE", "RECORD DE ZONA", "RECORD ZONA") + "\n" + action_text
		bridge.CHARACTER_SELECT_CONTEXT_TIME_ATTACK_BOSS:
			return description_text + "\n" + bridge._language_text("BOSS RECORD ATTACK", "BOSS-REKORDANGRIFF", "RECORD BOSS", "RECORD DE JEFE", "RECORD BOSS") + "\n" + action_text
		bridge.CHARACTER_SELECT_CONTEXT_MULTIPLAYER:
			return description_text + "\n" + bridge._language_text("LOCK THE REMATCH CHARACTER", "RUCKKAMPF-CHARAKTER FESTLEGEN", "VERROUILLEZ LE PERSONNAGE", "FIJA EL PERSONAJE", "BLOCCA IL PERSONAGGIO") + "\n" + action_text
	return description_text + "\n" + action_text
