...
static func title(bridge: Object) -> String:
	return bridge._language_text("COMMUNICATION", "KOMMUNIKATION", "COMMUNICATION", "COMUNICACION", "COMUNICAZIONE") if bridge._title_phase == bridge.TITLE_PHASE_MULTI_CONNECT else bridge._language_text("SINGLE-PAK SYNC", "SINGLE-PAK-SYNC", "SYNC SINGLE-PAK", "SYNC SINGLE-PAK", "SYNC SINGLE-PAK")

static func prompt(bridge: Object) -> String:
	if not bridge._title_notice_text.is_empty():
		return bridge.get_title_notice_text()
	if bridge._title_phase == bridge.TITLE_PHASE_MULTI_CONNECT:
		if bridge._multiplayer_pak_mode == 0:
			if bridge._multiplayer_link_ready:
				return bridge._language_text("PRESS START ON THE HOST TO BEGIN", "START AM HOST ZUM BEGINN DRUECKEN", "APPUYEZ SUR START SUR L'HOTE", "PULSA START EN EL HOST PARA EMPEZAR", "PREMI START SULL'HOST PER INIZIARE")
			if bridge.get_multiplayer_link_count() > 1:
				return bridge._language_text("WAITING FOR THE HOST TO CONFIRM", "WARTE AUF HOST-BESTAETIGUNG", "EN ATTENTE DE LA CONFIRMATION DE L'HOTE", "ESPERANDO CONFIRMACION DEL HOST", "IN ATTESA DELLA CONFERMA DELL'HOST")
			return bridge._language_text("WAITING FOR OTHER PLAYERS", "WARTE AUF ANDERE SPIELER", "EN ATTENTE DES AUTRES JOUEURS", "ESPERANDO A OTROS JUGADORES", "IN ATTESA DEGLI ALTRI GIOCATORI")
		if not bridge.is_singlepak_transfer_started():
			return bridge._language_text("WAIT FOR CLIENT SYSTEMS TO JOIN", "WARTE AUF CLIENT-SYSTEME", "EN ATTENTE DES SYSTEMES CLIENTS", "ESPERANDO A LOS CLIENTES", "IN ATTESA DEI CLIENT")
		return bridge._language_text("SENDING THE CLIENT PROGRAM", "CLIENT-PROGRAMM WIRD GESENDET", "ENVOI DU PROGRAMME CLIENT", "ENVIANDO EL PROGRAMA CLIENTE", "INVIO DEL PROGRAMMA CLIENT")
	return bridge._language_text("WAIT FOR CLIENT BOOT TO COMPLETE", "WARTE AUF CLIENT-START", "ATTENDEZ LE DEMARRAGE DU CLIENT", "ESPERA EL ARRANQUE DEL CLIENTE", "ATTENDI L'AVVIO DEL CLIENT")

static func prompt_color(bridge: Object) -> Color:
	return Color(0.88, 0.44, 0.18, 1.0) if bridge._title_phase == bridge.TITLE_PHASE_MULTI_CONNECT else Color(0.72, 0.28, 0.22, 1.0)
...
static func info(bridge: Object) -> String:
	if bridge._title_phase == bridge.TITLE_PHASE_MULTI_CONNECT:
		if bridge._multiplayer_pak_mode == 0:
			return bridge._language_text("FORM A MULTI-PAK ROOM FOR %s", "MULTI-PAK-RAUM FUER %s BILDEN", "FORMER UNE SALLE MULTI-PAK POUR %s", "CREAR UNA SALA MULTI-PAK PARA %s", "CREA UNA STANZA MULTI-PAK PER %s") % bridge.get_multiplayer_session_course_text()
		if not bridge.is_singlepak_transfer_started():
			return bridge._language_text("WAIT FOR CLIENTS, THEN SEND THE MULTIBOOT PROGRAM", "AUF CLIENTS WARTEN, DANN MULTIBOOT SENDEN", "ATTENDRE LES CLIENTS PUIS ENVOYER LE MULTIBOOT", "ESPERA A LOS CLIENTES Y ENVIA EL MULTIBOOT", "ATTENDI I CLIENT E INVIA IL MULTIBOOT")
		return bridge._language_text("TRANSFER THE CLIENT PROGRAM FOR %s", "CLIENT-PROGRAMM FUER %s UEBERTRAGEN", "TRANSFERER LE PROGRAMME CLIENT POUR %s", "TRANSFERIR EL PROGRAMA CLIENTE PARA %s", "TRASFERISCI IL PROGRAMMA CLIENT PER %s") % bridge.get_multiplayer_session_course_text()
	return bridge._language_text("WAIT FOR CLIENT BOOT AND FINAL SYNCHRONIZATION", "WARTE AUF CLIENT-START UND SYNC", "ATTENDRE LE DEMARRAGE ET LA SYNCHRONISATION", "ESPERA EL ARRANQUE Y LA SINCRONIZACION", "ATTENDI AVVIO E SINCRONIZZAZIONE DEL CLIENT")
...
static func summary(bridge: Object) -> String:
	...
	var clients_label: String = bridge._language_text("CLIENTS", "CLIENTS", "CLIENTS", "CLIENTES", "CLIENT")
	var course_label: String = bridge._language_text("COURSE", "KURS", "PARCOURS", "FASE", "ZONA")
	var state_label: String = bridge._language_text("STATE", "STATUS", "ETAT", "ESTADO", "STATO")
	var linked_label: String = bridge._language_text("LINKED", "VERBUNDEN", "LIES", "ENLAZADOS", "COLLEGATI")
	var ready_label: String = bridge._language_text("READY", "BEREIT", "PRETS", "LISTOS", "PRONTI")
	var waiting_label: String = bridge._language_text("WAITING", "WARTEN", "ATTENTE", "ESPERA", "ATTESA")
	var booting_label: String = bridge._language_text("BOOTING", "STARTET", "DEMARRAGE", "ARRANCANDO", "AVVIO")
	if bridge._title_phase == bridge.TITLE_PHASE_MULTI_CONNECT:
		if bridge._multiplayer_pak_mode == 0:
	...
	return bridge._language_text("SYNC STEP", "SYNC-SCHRITT", "ETAPE SYNC", "PASO SYNC", "PASSO SYNC") + "\n%d/3\n\n%s\n%s\n\n%s\n%s" % [bridge.get_singlepak_sync_step(), course_label, bridge.get_selected_level_text(), state_label, ready_label if bridge._singlepak_sync_step >= 3 else booting_label]

static func signal_text(bridge: Object) -> String:
	return bridge._language_text("LINK", "LINK", "LIAISON", "ENLACE", "LINK") if bridge._title_phase == bridge.TITLE_PHASE_MULTI_CONNECT else bridge._language_text("SYNC", "SYNC", "SYNC", "SYNC", "SYNC")

static func section_text(bridge: Object) -> String:
	return bridge._language_text("PLAYER STATUS", "SPIELERSTATUS", "STATUT JOUEUR", "ESTADO DEL JUGADOR", "STATO GIOCATORE")
