...
static func time_records_character(bridge: Object) -> String:
	...
	return "SONIC" if rows.is_empty() else rows[clampi(bridge._time_records_character_index, 0, rows.size() - 1)]

static func time_records_course_heading(bridge: Object) -> String:
	if bridge._time_records_view == bridge.TIME_RECORDS_VIEW_MODE_CHOICE:
		return bridge._language_text("MODE SELECT", "MODUS WAEHLEN", "CHOIX DU MODE", "ELEGIR MODO", "SCELTA MODALITA")
	var zone_number: int = bridge._time_records_course_index + 1
	if bridge._time_records_boss_mode:
		return "%s %d   %s" % [bridge._language_text("ZONE", "ZONE", "ZONE", "ZONA", "ZONA"), zone_number, bridge._language_text("BOSS", "BOSS", "BOSS", "JEFE", "BOSS")]
	return "%s %d   %s %d" % [bridge._language_text("ZONE", "ZONE", "ZONE", "ZONA", "ZONA"), zone_number, bridge._language_text("ACT", "AKT", "ACTE", "ACTO", "ATTO"), bridge._time_records_act_index + 1]

static func time_records_course_subtitle(bridge: Object) -> String:
	if bridge._time_records_view == bridge.TIME_RECORDS_VIEW_MODE_CHOICE:
		return bridge._language_text("CHOOSE ZONE OR BOSS RECORDS", "ZONEN- ODER BOSS-REKORDE WAEHLEN", "CHOISIR RECORDS ZONE OU BOSS", "ELEGIR RECORDS DE ZONA O JEFE", "SCEGLI RECORD ZONA O BOSS")
	var course_name: String = bridge.get_level_name_by_index(bridge._get_time_records_level_index())
	return "%s %s" % [course_name, bridge._language_text("BOSS ROUTE", "BOSS-ROUTE", "PARCOURS BOSS", "RUTA DE JEFE", "PERCORSO BOSS")] if bridge._time_records_boss_mode else course_name

static func time_records_best_label(bridge: Object, index: int) -> String:
	return "%s %d" % [bridge._language_text("BEST", "BESTE", "MEILLEUR", "MEJOR", "MIGLIORE"), index + 1]
...
static func multiplayer_records_title(bridge: Object) -> String:
	return bridge._language_text("VS RECORDS", "VS-REKORDE", "RECORDS VS", "RECORDS VS", "RECORD VS")
