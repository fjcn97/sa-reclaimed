...
static func progress(bridge: Object) -> float:
	if not bridge.is_time_attack_clear_screen():
		return 0.0
	return clampf(1.0 - (bridge.get_clear_result_state().input_lock_timer / 2.666), 0.0, 1.0)

static func title_text(bridge: Object) -> String:
	return bridge.language_text("TIME ATTACK RESULTS", "TIME ATTACK ERGEBNIS", "RESULTAT TIME ATTACK", "RESULTADOS TIME ATTACK", "RISULTATI TIME ATTACK")

static func time_text(bridge: Object) -> String:
	return bridge.get_formatted_time(bridge.get_clear_result_state().time_snapshot)
