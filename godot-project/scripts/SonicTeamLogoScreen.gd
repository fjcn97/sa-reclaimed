...
func _ready() -> void:
	...
	_ensure_chrome()
	_set_screen_visible(_bridge != null and _bridge.is_sonic_team_logo_screen())

func _process(delta: float) -> void:
	if _bridge == null:
		_bridge = resolve_state_bridge()
	var active: bool = _bridge != null and _bridge.is_sonic_team_logo_screen()
	...
	if title_label:
		title_label.text = _bridge.get_sonic_team_logo_title_text()
	...
	if prompt_label:
		prompt_label.text = _bridge.get_sonic_team_logo_prompt_text()
	...
	if detail_label:
		detail_label.text = _bridge.get_sonic_team_logo_detail_text()
...
func _ensure_chrome() -> void:
	_backdrop = ensure_rect("BackdropShade", Rect2(0.0, 0.0, 1280.0, 720.0), Color(0.02, 0.04, 0.11, 0.96))
	_panel = ensure_rect("Panel", Rect2(180.0, 210.0, 920.0, 230.0), Color(0.08, 0.16, 0.30, 0.90))
	_accent = ensure_rect("Accent", Rect2(220.0, 462.0, 840.0, 6.0), Color(0.30, 0.76, 1.0, 0.44))
...
func _update_source_logo() -> void:
	...
	if _source_texture == null:
		_source_texture = SourceTilemapTextureImpl.compose(_bridge.get_sonic_team_logo_source_tilemap())
