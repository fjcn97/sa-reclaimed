...
static func update(bridge: Object, level: LevelState, delta: float) -> void:
	for entity in level.entities:
		...
		if entity.type == ENTITY_TYPES.ENTITY_TRAPPED_ANIMAL:
			bridge.update_trapped_animal_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_RING_EFFECT:
			bridge.update_ring_effect_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_HEART_EFFECT:
			bridge.update_heart_effect_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_DUST_EFFECT:
			bridge.update_dust_effect_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_GRIND_EFFECT:
			bridge.update_grind_effect_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_CHEESE:
			bridge.update_cheese_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_TAIL_SWIPE or entity.type == ENTITY_TYPES.ENTITY_KNUCKLES_FIRE or entity.type == ENTITY_TYPES.ENTITY_SONIC_SKID:
			bridge.update_character_attack_effect(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_ITEM_BOX:
			bridge.update_item_box_effect(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_SPECIAL_RING:
			bridge.update_special_ring_motion(entity, delta)
			continue
		...
		if entity.type == ENTITY_TYPES.ENTITY_SCATTER_RING:
			bridge.update_scattered_ring(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_BUZZER:
			bridge.update_buzzer_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_BALLOON:
			bridge.update_balloon_motion(entity, delta)
			continue
		...
		if entity.type == ENTITY_TYPES.ENTITY_BULLET_BUZZER:
			bridge.update_bullet_buzzer_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_KOURA:
			bridge.update_koura_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_STAR:
			bridge.update_star_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_KIKI:
			bridge.update_kiki_motion(entity, delta)
			continue
		if entity.enemy_profile == 1:
			bridge.update_pen_motion(entity, delta)
			continue
		if entity.enemy_profile == 2:
			bridge.update_bell_motion(entity, delta)
			continue
		if entity.enemy_profile == 3:
			bridge.update_mouse_motion(entity, delta)
			continue
		if entity.enemy_profile == 4:
			bridge.update_circus_motion(entity, delta)
			continue
		if entity.enemy_profile == 5:
			bridge.update_yado_motion(entity, delta)
			continue
		if entity.enemy_profile == 6:
			bridge.update_gohla_motion(entity, delta)
			continue
		if entity.enemy_profile == 7:
			bridge.update_hammerhead_motion(entity, delta)
			continue
		if entity.enemy_profile == 8:
			bridge.update_kura_kura_motion(entity, delta)
			continue
		if entity.enemy_profile == 10:
			bridge.update_gejigeji_motion(entity, delta)
			continue
		if entity.enemy_profile == 11:
			bridge.update_kubinaga_motion(entity, delta)
			continue
		if entity.enemy_profile == 12:
			bridge.update_madillo_motion(entity, delta)
			continue
		if entity.enemy_profile == 13:
			entity.state_timer = fmod(entity.state_timer + delta * 4.0, TAU)
			continue
		if entity.enemy_profile == 14:
			bridge.update_kyura_motion(entity, delta)
			continue
		if entity.enemy_profile == 15:
			bridge.update_flickey_motion(entity, delta)
			continue
		if entity.enemy_profile == 16:
			bridge.update_mon_motion(entity, delta)
			continue
		if entity.enemy_profile == 18:
			bridge.update_pikopiko_motion(entity, delta)
			continue
		if entity.enemy_profile == 9:
			bridge.update_straw_motion(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_KIKI_PROJECTILE:
			bridge.update_kiki_projectile(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_KIKI_PIECE:
			bridge.update_kiki_piece(entity, delta)
			continue
		if entity.type == ENTITY_TYPES.ENTITY_BOSS:
			var player := bridge.get_player_state() as PlayerState
			var elapsed: float = bridge.get_gameplay_runtime_state().elapsed_time
