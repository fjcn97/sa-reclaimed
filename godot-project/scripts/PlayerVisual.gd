...
func _ready() -> void:
	...
	visible = _bridge != null and _bridge.should_show_player_visual()
...
func _process(delta: float) -> void:
	...
	visible = state.is_alive and _bridge.should_show_player_visual()
	if not visible:
		_hide_boost_trails()
		return

	_body.modulate = _bridge.get_player_visual_color(state.variant, false)
	...
	if state.has_cleared_level:
		_body.modulate = _bridge.get_player_visual_color(state.variant, true)
...
func _draw() -> void:
	...
	if _bridge.is_player_invincible():
		for i in range(8):
	...
	if _bridge.is_player_speed_up_active():
		for i in range(3):
...
func _update_boost_trails() -> void:
	if _bridge == null or not _bridge.is_player_boosting():
		_hide_boost_trails()
		return
	var positions: Array = _bridge.get_boost_trail_positions()
