...
func _ready() -> void:
	...
	var chrome_nodes := HUD_CHROME_BUILDER.build(self, _bridge.get_hud_chrome_colors() if _bridge else {})
	...
	_race_start_label.text = _bridge.get_hud_race_start_text() if _bridge else ""
...
func _process(_delta: float) -> void:
	if _bridge == null:
		...
		if _bridge == null:
			...
			return
	var title_mode: bool = _bridge.is_title_screen()
	var pause_mode: bool = _bridge.is_paused()
	var save_mode: bool = _bridge.is_save_options()
	var intro_mode: bool = _bridge.is_intro_screen()
	var clear_mode: bool = _bridge.is_clear_screen()
	var special_stage_mode: bool = _bridge.is_special_stage_screen()
	var presentation_mode: bool = _bridge.is_game_over_screen() \
		or _bridge.is_final_intro_screen() \
		or _bridge.is_chaos_emeralds_screen() \
		or _bridge.is_missing_emeralds_screen() \
		or _bridge.is_to_be_continued_screen() \
		or _bridge.is_sega_logo_screen() \
		or _bridge.is_sonic_team_logo_screen() \
		or _bridge.is_credits_screen() \
		or _bridge.is_copyright_screen() \
		or _bridge.is_credits_end_screen() \
		or _bridge.is_character_unlock_screen()
	...
	var elapsed = _bridge.get_elapsed_time()

	_set_hud_visible(hud_visible)
	if not hud_visible:
		return
	if _race_start_label:
		var race_start_visible: bool = _bridge.is_race_start_message_visible()
		var race_start_progress: float = _bridge.get_race_start_message_progress()
		var remaining_frames: float = race_start_progress * 60.0
	...
	var chrome: Dictionary = _bridge.get_hud_chrome_colors()
	if score_label:
	...
	if special_ring_label:
		special_ring_label.visible = _bridge.is_special_ring_hud_visible()
		special_ring_label.text = _bridge.get_hud_special_ring_text()
	...
	if powerup_label:
		powerup_label.text = _bridge.get_hud_powerup_text()
		powerup_label.visible = powerup_label.text != ""
		if _bridge.is_player_magnetic_shielded():
			powerup_label.modulate = Color(0.78, 0.58, 1.0, 1.0)
		elif _bridge.is_hud_shield_active():
			powerup_label.modulate = Color(0.52, 0.92, 1.0, 1.0)
		elif _bridge.is_player_invincible():
			powerup_label.modulate = Color(1.0, 0.92, 0.30, 1.0)
		elif _bridge.is_player_speed_up_active():
	...
	if time_label:
		time_label.text = _bridge.get_hud_time_text()
		...
		if _bridge.is_hud_timer_warning():
			time_color = Color(1.0, 0.30, 0.22, 0.52 + absf(sin(Time.get_ticks_msec() / 180.0)) * 0.48)
		time_label.modulate = time_color
	if status_label:
		status_label.text = _bridge.get_status_text()
		status_label.visible = _bridge.get_status_text() != ""
...
func _update_boss_panel(chrome: Dictionary) -> void:
	var boss_state: Dictionary = _bridge.get_boss_hud_state()
	...
	if _boss_title:
		...
		_boss_title.text = _bridge.get_hud_boss_title_text(health, max_health)
		_boss_title.modulate = Color(1.0, 0.82, 0.62, 1.0)
	if _boss_phase:
		_boss_phase.visible = boss_visible
		_boss_phase.text = _bridge.get_hud_boss_phase_text(str(boss_state.get("phase", "")))
...
func _update_titles(character_variant: int, chrome: Dictionary) -> void:
	var titles: Dictionary = _bridge.get_hud_titles()
	if _score_title:
	...
	if _character_label:
		_character_label.text = _bridge.get_hud_character_short_name(character_variant)
...
func _update_multiplayer_panel(chrome: Dictionary) -> void:
	var multiplayer_visible: bool = _bridge.is_multiplayer_run()
	if _mp_panel:
	...
	if _mp_start_flag:
		...
		_mp_start_flag.text = _bridge.get_hud_multiplayer_start_flag_text()
		_mp_start_flag.modulate = chrome.get("score_title", Color(1.0, 0.82, 0.52, 0.94))
	if _mp_finish_flag:
		_mp_finish_flag.visible = multiplayer_visible
		_mp_finish_flag.text = _bridge.get_hud_multiplayer_finish_flag_text()
		_mp_finish_flag.modulate = chrome.get("score_title", Color(1.0, 0.82, 0.52, 0.94))

	var rows: Array = _bridge.get_multiplayer_hud_rows()
	for i in range(_mp_row_cards.size()):
		var has_row: bool = multiplayer_visible and i < rows.size() and rows[i] is Dictionary
		var row: Dictionary = {}
		if has_row:
...
func _set_hud_visible(hud_visible: bool) -> void:
	...
	if _special_ring_card:
		_special_ring_card.visible = hud_visible and _bridge.is_special_ring_hud_visible()
	if _lives_glow:
	...
	if special_ring_label:
		special_ring_label.visible = hud_visible and _bridge.is_special_ring_hud_visible()
	if powerup_label:
	...
	if status_label:
		status_label.visible = hud_visible and _bridge.get_status_text() != ""
	if _race_start_label:
		_race_start_label.visible = hud_visible and _bridge.is_race_start_message_visible()
	if _score_title:
	...
	if _boss_panel:
		_boss_panel.visible = hud_visible and _bridge.get_boss_hud_state().get("active", false)
	if _boss_title:
		_boss_title.visible = hud_visible and _bridge.get_boss_hud_state().get("active", false)
	if _boss_phase:
		_boss_phase.visible = hud_visible and _bridge.get_boss_hud_state().get("active", false)
	if _boss_health_back:
		_boss_health_back.visible = hud_visible and _bridge.get_boss_hud_state().get("active", false)
	if _boss_health_fill:
		_boss_health_fill.visible = hud_visible and _bridge.get_boss_hud_state().get("active", false)
	for pip in _boss_health_pips:
		pip.visible = hud_visible and _bridge.get_boss_hud_state().get("active", false)
	if not hud_visible:
		if _mp_panel:
