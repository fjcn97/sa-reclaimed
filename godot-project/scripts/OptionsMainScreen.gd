...
func _process(_delta: float) -> void:
	...
	if prompt_label:
		prompt_label.text = CoreBridge.get_options_main_prompt_text()
	...
	if detail_label:
		detail_label.text = "%s\n%s" % [CoreBridge.get_options_summary_text().replace("   ", "   "), CoreBridge.get_options_main_detail_text()]
...
func _ensure_chrome() -> void:
	...
	_hero_glow = _ensure_rect("HeroGlow", Rect2(104.0, 96.0, 1072.0, 504.0), Color(0.22, 0.68, 0.90, 0.10))
	...
	_accent = _ensure_rect("AccentBar", Rect2(118.0, 150.0, 1044.0, 10.0), Color(0.24, 0.78, 0.96, 0.96))
	_header_band = _ensure_rect("HeaderBand", Rect2(160.0, 214.0, 412.0, 258.0), Color(0.90, 0.97, 1.0, 0.98))
	_menu_stage = _ensure_rect("MenuStage", Rect2(160.0, 214.0, 412.0, 258.0), Color(0.90, 0.97, 1.0, 0.98))
	_summary_stage = _ensure_rect("SummaryStage", Rect2(610.0, 214.0, 236.0, 258.0), Color(0.24, 0.78, 0.96, 0.94))
	_badge_ring = _ensure_rect("BadgeRing", Rect2(892.0, 224.0, 220.0, 220.0), Color(0.24, 0.78, 0.96, 0.22))
...
func _ensure_rows() -> void:
	...
	for i in range(8):
		...
		var card := _ensure_rect("RowCard%d" % i, Rect2(188.0, top, 356.0, 24.0), Color(0.88, 0.95, 1.0, 1.0))
		var row := _ensure_label("RowLabel%d" % i, Vector2(208.0, top - 1.0), Vector2(146.0, 24.0), 15)
		var value := _ensure_label("ValueLabel%d" % i, Vector2(354.0, top - 1.0), Vector2(116.0, 24.0), 11)
		var status := _ensure_label("StatusLabel%d" % i, Vector2(470.0, top - 1.0), Vector2(54.0, 24.0), 11)
...
func _update_rows() -> void:
	...
	for i in range(_row_labels.size()):
		...
		_row_cards[i].size = Vector2(368.0, 26.0) if is_selected else Vector2(356.0, 24.0)
