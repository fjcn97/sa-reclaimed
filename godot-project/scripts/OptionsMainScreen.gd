...
func _ensure_chrome() -> void:
	...
	_header_band = ensure_rect("HeaderBand", Rect2(260.0, 212.0, 760.0, 342.0), Color(0.90, 0.97, 1.0, 0.98))
	_menu_stage = ensure_rect("MenuStage", Rect2(260.0, 212.0, 760.0, 342.0), Color(0.90, 0.97, 1.0, 0.98))
	...
	_prompt_band = ensure_rect("PromptBand", Rect2(118.0, 534.0, 1044.0, 148.0), Color(0.96, 0.99, 1.0, 0.99))
	_backdrop.z_index = -9
...
func _ensure_rows() -> void:
	...
	for i in range(8):
		...
		var card := ensure_rect("RowCard%d" % i, Rect2(280.0, top, 720.0, 34.0), Color(0.88, 0.95, 1.0, 1.0))
		var row := ensure_label("RowLabel%d" % i, Vector2(306.0, top - 1.0), Vector2(310.0, 34.0), 19)
		var value := ensure_label("ValueLabel%d" % i, Vector2(636.0, top - 1.0), Vector2(338.0, 34.0), 16)
